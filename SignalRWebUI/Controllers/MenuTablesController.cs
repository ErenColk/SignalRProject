...
namespace SignalRWebUI.Controllers
{
    public class MenuTablesController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7194/api/MenuTables"); //Tetiklenecek yer 

            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync(); // Jsondan gelen içeriği string formatında oku 
                var values = JsonConvert.DeserializeObject<List<ResultMenuTableDto>>(jsonData); // Burada okunan değeri Json Formatından deserilaze ederek normal metne döndürür.
        ...
        public async Task<IActionResult> CreateMenuTable(CreateMenuTableDto createMenuTableDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7194/api/MenuTables", stringContent); // API'ye git ve giderken olusturdugumuz kaynağı götür.
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7194/api/MenuTables/{id}");
        ...
        public async Task<IActionResult> UpdateMenuTable(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7194/api/MenuTables/{id}");
        ...
        public async Task<IActionResult> UpdateMenuTable(UpdateMenuTableDto updateMenuTableDto)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7194/api/MenuTables", stringContent);
